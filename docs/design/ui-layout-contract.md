...
- **Árbol de conexiones:** carpetas con chevron Lucide + icono carpeta + nombre + `+` (nueva conexión); sin basurero en la fila. Filas de carpeta compactas (densidad de lista, sin chrome de tarjeta). Conexiones indentadas con guía vertical (`border-left`). Acción add-folder (FolderPlus). Rename de carpeta: menú contextual → input inline (Enter/Escape/blur); no doble clic. Eliminar carpeta: menú contextual → dialog glass A1 con conteo (cascade). **Click primario en toda la fila** de carpeta alterna expand/collapse; el botón `+` usa `stopPropagation` para no disparar el toggle.
- **Cajita de conexión:** item con chrome de tarjeta (fondo sutil, borde sakura, `border-radius`, padding) bajo `.connection-tree .profile-item`; nombre + `user@host:port` en cyan; sin label de auth/túnel. Carpetas siguen compactas (sin tarjeta); indent + guía vertical (`border-left`) en hijos. Sin lápiz ni basurero inline. Icono Copy copia `user@host`. **Doble clic** abre la sesión SSH; click simple solo resalta. Editar (modal), renombrar (inline) y eliminar (confirm A1) viven en el menú contextual B3. Nota: el preview `preview-connection-tree-dense.html` documentó un experimento de filas densas; el producto restaura cajitas (paridad con estilos de `main` en items).
