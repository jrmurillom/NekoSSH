...
## General rules
- **Español latino** para documentación legible por el equipo (`docs/`, OpenSpec, README, comentarios explicativos).
- **Inglés** solo para identificadores técnicos embebidos (nombres de rutas, columnas SQL, archivos de código).
- Al crear o actualizar docs, mantener consistencia con `docs/base-standards.md`.
...
2. Identify which documentation files need updates based on the changes. Some clear examples:
   ...
   - For visual/style changes (colors, typography, components): Update docs/design/DESIGN.md (not docs/design-system.md — that file is only an index)
   - For page structure changes (layout shell, zones, page patterns): Update docs/design/ui-layout-contract.md
   - For changes in libraries, database migrations, or anything that changes the installation process, update *-standards.md
3. Actualizar cada archivo afectado en **español latino**, manteniendo identificadores técnicos en inglés.
