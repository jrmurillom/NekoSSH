...
- **Feature/ticket identifiers**: e.g. ZenTao task ids (`task-456`, `456`), story ids (`story-12`), branch names, or short feature labels. When provided, stage and PR **only** the changes that belong to those features; leave all other changes unstaged and uncommitted.
...
- Escribir el mensaje de commit **en español latino** (según `docs/base-standards.md`). Identificadores técnicos en inglés cuando aplique.
- Make it **descriptive** (per Git Workflow in `backend-standards.md` and `frontend-standards.md`).
- Structure it so that:
  - **Subject line**: Short, imperative summary. Prefix with ZenTao id when applicable: `task-456: ...` (default) or `story-12: ...`.
  - **Body** (if needed): Bullet points or short paragraphs describing what changed and why (areas touched, new behavior, fixes). Reference ZenTao story ids and links here if they apply.
...
- Create or update the PR for the current branch:
  - **Title**: e.g. `[task-456] Add note color picker` or `[story-12] ...` when applicable.
  - **Description**: Summarize the change set, link to the ZenTao story if relevant, and note any testing or follow-ups.
